...
static loadResourcePromise(url){return new Promise(load);function load(fulfill,reject){const xhr=new XMLHttpRequest();xhr.open('GET',url,true);xhr.onreadystatechange=onreadystatechange;const requestStart=performance.now();function onreadystatechange(e){if(xhr.readyState!==XMLHttpRequest.DONE)
return;const status=/^HTTP\/1.1 404/.test(e.target.response)?404:xhr.status;if(url.indexOf('remote')>-1){const requestDone=performance.now();InspectorFrontendHost.recordPerformanceHistogram('DevTools.FrontendRemoteRequestLoadTime',(requestDone-requestStart));}
