...
function requestHandler(request, response) {
	...
	if(filePath == '/json/new'){
		console.log(parseURL(request.headers.host),'||||||||||||||||||||||')
		fileSearch = parseURL(request.url).search;
	}
	var proxiedFile = proxy(filePath, sendResponse);
	if (proxiedFile && filePath == '/json/new') {
...
function proxy(filePath) {
	...
	if (filePath == '/json/new')
		console.log(fileSearch,"============",`http://localhost:9460/json/new${fileSearch}`)
		return utils.fetch(`http://localhost:9460/json/new${fileSearch}`)
